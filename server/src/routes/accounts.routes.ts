...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const service = new AccountsService(user?.id);
...
router.get(
  ...
  (req: Request, res: Response) => {
    ...
    console.log("[accounts] list returned", {
...
router.get(
  ...
  (req: Request, res: Response) => {
    ...
    console.log("[accounts] get requested", {
      id: req.params.id,
      found: !!account,
      userId: user?.id,
    });
...
router.delete(
  ...
  (req: Request, res: Response) => {
    ...
    console.log("[accounts] delete requested", {
      id: req.params.id,
      deleted,
      userId: user?.id,
    });
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const service = new AccountsService(user?.id);
      console.log("[accounts] refresh requested", {
        id: req.params.id,
        userId: user?.id,
      });
      ...
      console.error("[accounts] refresh failed", {
        id: req.params.id,
        message,
      });
      return res.status(400).json({ error: message });
...
router.patch(
  ...
  (req: Request, res: Response) => {
    ...
    console.log("[accounts] favorite toggled", {
      id: req.params.id,
      found: !!account,
      userId: user?.id,
    });
...
router.get(
  ...
  (req: Request, res: Response) => {
    ...
    console.log("[accounts] dashboard returned", {
